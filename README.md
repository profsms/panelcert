...
- `twfe_adequacy(y, unit, time, first_treat)` reports the saturated group-time
  ...
  normalization is the default; `cluster = "ar1"`, `cluster = "iid"`, and a
  positive user-supplied `psi` are sensitivity routes.
...
they do not assert that the realized effect profile is a worst-case direction.
...
canonical 11-firm Grunfeld showcase, are bundled for
